...
c  m newville:  relies on external istrln and echo routines
     ...
     $         'fit gave an impossible error message.')
       elseif ( (info.ge.4).and.(info.le.7)) then
          call warn(2,'           fit gave a warning message:')
          if (info.eq.4) then
             call warn(2,'      one or more '//
     $            'variables may not affect the fit.')
          elseif (info.eq.5) then
             call warn(2,'      too many fit '//
     $            'iterations.  try better guesses,')
             call warn(2,'      a simpler problem, or '//
     $            'increase "&max_iteration".')
          elseif ((info.eq.6).or.(info.eq.7)) then
             call warn(2,'      "toler" can probably be '//
     $            'increased without a loss of')
             write(messg, '(a,e13.5)' ) '      fit quality. '//
     $            'current value is:  toler = ', toler
             im = istrln(messg)
             call warn(2,messg(:im))
          endif
       end if
       return
       end
...
c#mn{
       double precision xiter, xmaxiter,getsca
       external getsca
c#mn}
...
c
c#mn{
c get maximum number of iterations from &max_iterations
       xmaxiter = getsca('&max_iteration',0)
       if (xmaxiter.lt.1.d0) xmaxiter = 250.d0
c#mn}
...
c print message to let user know that routine is running
         ...
         if (mod(iter,int(xmaxiter/10)).eq.0)
     $        call warn(1,'         fitting ...')
c#mn}
c
