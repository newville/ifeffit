...
C replaced use of %val() JBB-QSAS
      IF (NPICT.GT.1) THEN
         CALL GRGI10 (FILENM, NPICT, MSG)
         UNIT = GROFIL(MSG)
         IF (UNIT.LT.0) THEN
...
*+
      SUBROUTINE GRGI01 (IX0, IY0, IX1, IY1, ICOL, BX, BY, PIXMAP_PTR)
C replaced use of %val() JBB-QSAS
...
*+
      SUBROUTINE GRGI03 (IX0, IY0, IX1, IY1, ICOL, BX, BY, PIXMAP_PTR)
C replaced use of %val() JBB-QSAS
      INTEGER IX0, IY0, IX1, IY1
      INTEGER ICOL, BX, BY
      INTEGER PIXMAP_PTR
...
*+
      SUBROUTINE GRGI04(NBUF,RBUF,BX,BY,PIXMAP_PTR,MAXIDX)
C replaced use of %val() JBB-QSAS
...
*+
      SUBROUTINE GRGI06 (UNIT,BX,BY,CTABLE,PIXMAP_PTR,MAXIDX,CPTR)
C replaced use of %val() JBB-QSAS
