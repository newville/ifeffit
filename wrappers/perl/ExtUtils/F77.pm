...
# Associated compiler flags for shared libraries
## BR Jun 30 1999 I have only added entries for the two systems for
## which I actually know the answer -- g77 and irix

$F77config{Sunos}{F77}{Share} = '';
...
$F77config{Generic}{G77}{Share} = '-shared -fpic';
...
$F77config{Hpux}{F77}{Cflags} = '-O +z';
$F77config{Hpux}{F77}{Share} = '';
...
## probably this depends on the switches above also...
$F77config{Irix}{F77}{Share} = '-KPIC -shared';
...
EOD
   ...
   ## snippet added by BR Jun 30 1999
   if (defined( $F77config{$system}{$compiler}{Share} )) {
	$Share = $F77config{$system}{$compiler}{Share};
   } else {
	print << "EOD";
$Pkg: There does not appear to be any configuration info about
$Pkg: shared library options for the F77 compiler. Will assume
$Pkg: none necessary.
EOD
	$Share = '';
   }

   print "$Pkg: Share: $Share\n";
   ## end of snippet by BR
...
Returns compiler flags for building shared libraries.
